	...
	Convey("Given an rds instance", t, func() {
		...
		Convey("With an invalid backup window", func() {
			...
			Convey("When validating the rds instance", func() {
				...
				Convey("Then should return an error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "RDS Instance backup window: Time format must take the form of 'hh24:mi-hh24:mi'. i.e. '21:30-22:00'")
				})
			})
		})
		...
		Convey("With an invalid backup window - hour", func() {
			...
			Convey("When validating the rds instance", func() {
				...
				Convey("Then should return an error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "RDS Instance backup window: Time format invalid. Hour must be between 0 and 23 hours")
				})
			})
		})
		...
		Convey("With an invalid backup window - minute", func() {
			...
			Convey("When validating the rds instance", func() {
				...
				Convey("Then should return an error", func() {
					So(err, ShouldNotBeNil)
					So(err.Error(), ShouldEqual, "RDS Instance backup window: Time format invalid. Minute must be between 0 and 59 minutes")
				})
			})
		})
