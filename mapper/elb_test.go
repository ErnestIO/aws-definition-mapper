	...
	Convey("Given a valid input definition", t, func() {
		...
		Convey("When i try to map elbs", func() {
			...
			Convey("Then it should map input elb rules", func() {
				...
				So(e[0].Name, ShouldEqual, "test")
				So(len(e[0].NetworkAWSIDs), ShouldEqual, 1)
				So(e[0].NetworkAWSIDs[0], ShouldEqual, `$(networks.items.#[name="web-nw"].network_aws_id)`)
				So(len(e[0].InstanceAWSIDs), ShouldEqual, 2)
				So(e[0].InstanceAWSIDs[0], ShouldEqual, `$(instances.items.#[name="web-1"].instance_aws_id)`)
				So(e[0].InstanceAWSIDs[1], ShouldEqual, `$(instances.items.#[name="web-2"].instance_aws_id)`)
				So(len(e[0].SecurityGroupAWSIDs), ShouldEqual, 1)
				So(e[0].SecurityGroupAWSIDs[0], ShouldEqual, `$(firewalls.items.#[name="web-sg"].security_group_aws_id)`)
